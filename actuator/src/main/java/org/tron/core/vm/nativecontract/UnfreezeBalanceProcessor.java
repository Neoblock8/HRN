...
public class UnfreezeBalanceProcessor {
  ...
  public long execute(UnfreezeBalanceParam param, Repository repo) {
    ...
    switch (param.getResourceType()) {
      case BANDWIDTH:
        repo.addTotalNetWeight(-unfreezeBalance / HRN_PRECISION);
        break;
      case ENERGY:
        repo.addTotalEnergyWeight(-unfreezeBalance / HRN_PRECISION);
        break;
      default:
        //this should never happen
        break;
    ...
    if (VMConfig.allowTvmVote() && !accountCapsule.getVotesList().isEmpty()) {
      ...
      if (accountCapsule.getTronPower() < usedTronPower * HRN_PRECISION) {
