...
Having a tailored actuator is key to building a hrnetwork-based customized public chain. This article illustrates how to develop a hrnetwork-based `SumActuator`.
...
Currently, contracts supported by HRN are defined under `src/main/protos/core/contract` directory in protocol module. First creating a `math_contract.proto` file under this directory and declaring `SumContract`. You can also implement any mathematical calculation you want, such as `Subtraction`.
...
```
At last, recompile the modified proto files. Compiling the HRN project directly will compile the proto files as well, `protoc` command is also supported.

*Currently, HRN uses protoc v3.4.0. Please keep the same version when compiling by `protoc` command.*
...
For now, the default Actuator supported by HRN is located in `org.tron.core.actuator`. Creating `SumActuator` under this directory:
