...
# Function: to start, stop and restart HRNetwork.
...
case "${1}" in
    start)
        ...
        if [ $? -eq 0 ]; then
            echo "Succeeded to start HRNetwork."
        else
            echo "Failed to start HRNetwork."
        fi
    ;;
    stop)
        ...
        if [ $? -eq 0 ]; then
            rm ${PID_FILE_PATH}
            echo "Succeeded to stop HRNetwork."
        else
            echo "Failed to stop HRNetwork."
        fi
    ;;
    restart)
